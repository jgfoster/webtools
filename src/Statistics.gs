...
statistics
	...
	| path oop statNames file process statistics flotData options |
	path := arguments at: 'path' ifAbsent: [self error: 'Path is missing!'].
	oop := (arguments at: 'processOop' ifAbsent: [self error: 'Process is missing!']) asNumber.
	statNames := arguments at: 'statNames' ifAbsent: [self error: 'Statistic names are missing!'].
	statNames isEmpty ifTrue: [self error: 'No statistic names!'].
	file := Statmonitor fileAt: path.
	process := file processes
		detect: [:each | each asOop = oop]
		ifNone: [self error: 'Process not found!'].
	statistics := process statistics select: [:each | statNames includes: each name].
	statistics size == statNames size ifFalse: [self error: 'Some statistic not found!'].
	flotData := statistics collect: [:each | 
		Dictionary new
			at: 'data' put: each flotData;
			at: 'label' put: each name;
			yourself.
	].
	options := Dictionary new
		at: 'xaxis' 	put: (Dictionary new at: 'mode' put: 'time'; yourself);
	...
	^Dictionary new
		...
		at: 'flotData'		put: flotData;
		at: 'options'		put: options;
		at: 'tzOffset' 		put: file timeZoneOffset * 1000;
		yourself.
%
doit
