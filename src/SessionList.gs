...
sortOrder
	...
	^(System myUserProfile privileges includes: #'SessionAccess')
		ifTrue: [2]
		ifFalse: [nil].
%
...
statsForSlot
	...
	1 to: descriptions size do: [:i | 
		...
		type := metadata at: 2.
		value := statistics at: i.
		(type = #'svalue' and: [value notNil and: [0 < value]]) ifTrue: [
			bytes unsigned32At: 1 put: value.
			value := bytes signed32At: 1.
		].
		(type = #'svalue64' and: [value notNil and: [0 < value]]) ifTrue: [
			bytes unsigned64At: 1 put: value.
			value := bytes signed64At: 1.
		].
...
_sessionList
	...
	| sessions now size |
	size := self _sessionListLabels size.
	sessions := System currentSessions collect: [:each | 
		| list |
		list := System descriptionOfSession: each.
		list ifNotNil: [	"session might have logged out before we got info"
			list size: size.
			list at: list size put: (System cacheSlotForSessionId: each).
		].
		list.
	].
	...
	sessions := sessions collect: [:list | 
		...
		list
			...
			at: 14 	put: ((x := list at: 14) isNil ifTrue: [''] ifFalse: [(now - x) printString , 's']);		"Quiet"
			at: 15 	put: ((x := list at: 15) isNil ifTrue: [''] ifFalse: [(now - (list at: 15)) printString , 's']);		"Age"
			yourself.
	].
	0 timesRepeat: [sessions := sessions , sessions].	"to allow testing of scrolling when we have less than a page"
	^sessions.
%
category: 'Private'
set compile_env: 0
...
_sessionListLabels
	...
	^(Array new: 17)
		...
		at: 17 	put: 'Description' -> 'Nil or a String describing a system or gc gem.';
		at: 18 	put: 'Objects' -> 'Number of temporary (uncommitted) object IDs allocated to the session.';
		at: 19 	put: 'Pages' -> 'Number of temporary (non-persistent) page IDs allocated to the session.';
		at: 20 	put: 'Vote' -> '0 if session has not voted, 1 if session voting in progress, 2 if session has voted, or voting not active.';
		at: 21	put: 'Slot' -> 'The session''s cache process slot number if it is connected to the same shared page cache as Server. A return of nil indicates the session could not be located (so the gem is likely on another host).';
		yourself.
%
doit
